...
save_dir = "/global/cfs/cdirs/e3sm/jpaige3/ESEm/CV_Saved_Model_Data_masked/10seeds_GPCNNRFMLRSR_raw"
    ...
    for k, (train_index, test_index) in enumerate(kf.split(ppe_params)):
        ...
        def evaluate_model(model_name, m_train, _v_train, m_test, _v_test):
            ...
            for i, name in enumerate(var_names):
                ...
                results[f'{name}_train_r2_norm_raw'] = r2_n_raw
                results[f'{name}_train_r2_phys_raw'] = r2_p_raw
            ...
            for i, name in enumerate(var_names):
                ...
                results[f'{name}_test_r2_norm_raw'] = r2_n_raw
                results[f'{name}_test_r2_phys_raw'] = r2_p_raw
        ...
        gp_r2_file = os.path.join(
            save_dir, 'GP',
            f"GP_ZRG_r2output_uniform_avg_seed={seed}_k={k}_{timestamp}.json"
        )
        ...
        cnn_r2_file = os.path.join(
            save_dir, 'CNN',
            f"CNN_ZRG_r2output_uniform_avg_seed={seed}_k={k}_{timestamp}.json"
        )
        ...
        for i, name in enumerate(var_names):
            ...
            r2_p         = r2_score(var_trains[i], pred_train, multioutput='uniform_average')
            r2_p_raw     = r2_score(var_trains[i], pred_train, multioutput='raw_values')
            ...
            rf_results[f'{name}_train_r2_phys_raw'] = r2_p_raw

        print("\n  [RF] Out-of-sample metrics (physical):")
        for i, name in enumerate(var_names):
            pred_test    = pd.DataFrame(m_rf_test[:, :, i], index=X_test.index)
            r2_p         = r2_score(var_tests[i], pred_test, multioutput='uniform_average')
            r2_p_raw     = r2_score(var_tests[i], pred_test, multioutput='raw_values')
            ...
            rf_results[f'{name}_test_r2_phys_raw'] = r2_p_raw
        ...
        rf_r2_file = os.path.join(
            save_dir, 'RF',
            f"RF_ZRG_r2output_uniform_avg_seed={seed}_k={k}_{timestamp}.json"
        )
        ...
        mlr_r2_file = os.path.join(
            save_dir, 'MLR',
            f"MLR_ZRG_r2output_uniform_avg_seed={seed}_k={k}_{timestamp}.json"
        )
        ...
        spl_r2_file = os.path.join(
            save_dir, 'Spline',
            f"Spline_ZRG_r2output_uniform_avg_seed={seed}_k={k}_{timestamp}.json"
        )
