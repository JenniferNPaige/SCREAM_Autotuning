...
for size in SUBSET_SIZES:
    ...
    for seed_offset in range(N_SEEDS):
        ...
        for fold, (train_rel, test_rel) in enumerate(kf.split(subset_params)):
            ...
            # Compute out-of-sample R² per variable in normalized space
            for i, var in enumerate(VAR_NAMES):
                ...
                pred_norm   = m_test[:, :, i]
                r2 = r2_score(y_test_norm, pred_norm, multioutput='variance_weighted')
...
for size in SUBSET_SIZES:
    for var in VAR_NAMES:
        vals = seed_means[size][var]          # length = N_SEEDS
