...
Kernels evaluated
-----------------
  RBF           — Squared Exponential (default esem kernel)
  Matern12      — Matern ν=1/2  (equivalent to Ornstein–Uhlenbeck)
  Matern32      — Matern ν=3/2
  Matern52      — Matern ν=5/2  (smooth, widely used)
  RatQuad       — Rational Quadratic (mixture of RBFs at different length-scales)
  Linear        — Dot-product (linear) kernel
  RBF+Linear    — Additive: Squared Exponential + Linear
  Matern52+Lin  — Additive: Matern52 + Linear
"""
...
save_dir         = "/global/cfs/cdirs/e3sm/jpaige3/ESEm/CV_Saved_Model_Data_masked_GP_kernels"
    ...
    for seed_offset in range(N_SEEDS):
        ...
        for k, (train_index, test_index) in enumerate(kf.split(ppe_params)):
            ...
            result_file = os.path.join(
                out_dir,
                f"GP_{kernel_name}_ZRG_r2output_varweight_seed={seed}_k={k}_{timestamp}.json"
            )
